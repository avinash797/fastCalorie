...
export const POST = withAuth(async (request: NextRequest, admin, context) => {
  try {
    ...
    const newItemIndexes = itemIndexes.filter(
      (idx) => !existingApproved.has(idx),
    );
    ...
    const newApprovedIndexes = [
      ...new Set([...existingApproved, ...newItemIndexes]),
    ];
